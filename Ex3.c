...
int isGraphATree(node* nodelist, unsigned gorder) {
    int connectedComponents = isGraphConnected(nodelist, gorder);

    if (connectedComponents != 1) {
        return 0; // Not a tree if there are multiple connected components
    }

    // Reset the visited status of nodes
    for (unsigned i = 0; i < gorder; i++) {
    ...
    while (!IsEmpty(q)) {
        ...
        for (unsigned i = 0; i < nodelist[current].nedges; i++) {
            ...
            if (nodelist[neighbor].visited) {
                return 0; // Not a tree if a cycle is found
            }
            nodelist[neighbor].visited = 1;
            enqueue(neighbor, &q);
        }
    ...
    return 1; // It's a tree if there are no cycles
}
...
int main(int argc, char* argv[]) {
    ...
    int isTree = isGraphATree(nodelist, gorder);
    if (isTree) {
